...
def main(savedir):
    ...
    kwargs['title'] = 'Early Summer 2023'
    kwargs['ylabel'] = None
    ...
    pf.xsection(fig, ax5, spring.time.values, spring.depth_interpolated.values, spring.salinity.values, **kwargs)  

    kwargs['ylabel'] = None
    pf.xsection(fig, ax6, latespring.time.values, latespring.depth_interpolated.values, latespring.salinity.values, **kwargs)
    
    kwargs['colorbar'] = True
    pf.xsection(fig, ax7, summer_ph.time.values, summer_ph.depth_interpolated.values, summer_ph.salinity.values, **kwargs)
    ...
    pf.xsection(fig, ax9, spring.time.values, spring.depth_interpolated.values, spring.chlorophyll_a.values, **kwargs)  # spring chl
    ...
    pf.xsection(fig, ax13, spring.time.values, spring.depth_interpolated.values, placeholder, **kwargs)  # spring DO - not deployed

    kwargs['ylabel'] = None
    kwargs['xticklabels'] = None
    lsdo_mgL = latespring.oxygen_concentration_shifted.values * 32 / 1000  # convert from umol/L to mg/L
    pf.xsection(fig, ax14, latespring.time.values, latespring.depth_interpolated.values, lsdo_mgL, **kwargs)  # latespring DO

    kwargs['colorbar'] = True
    summerdo_mgL = summer_do.oxygen_concentration_shifted.values * 32 / 1000  # convert from umol/L to mg/L
    pf.xsection(fig, ax15, summer_do.time.values, summer_do.depth_interpolated.values, summerdo_mgL, **kwargs)  # summer DO
    ...
    pf.xsection(fig, ax17, spring.time.values, spring.depth_interpolated.values, spring.pH.values, **kwargs)  # spring pH

    kwargs['ylabel'] = None
    placeholder = np.full(np.shape(latespring.time.values), np.nan)
    pf.xsection(fig, ax18, latespring.time.values, latespring.depth_interpolated.values, placeholder, **kwargs)  # latespring pH - NOT DEPLOYED
    ...
    pf.xsection(fig, ax20, spring.time.values, spring.depth_interpolated.values, spring.aragonite_saturation_state.values, **kwargs)  # spring omega

    kwargs['ylabel'] = None
    pf.xsection(fig, ax21, latespring.time.values, latespring.depth_interpolated.values, placeholder, **kwargs)  # latespring omega - NOT DEPLOYED
