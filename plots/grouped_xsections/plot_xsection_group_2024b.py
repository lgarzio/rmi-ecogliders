...
def main(savedir):
    ...
    kwargs['colorbar'] = True
    pf.xsection(fig, ax7, winter_ph.time.values, winter_ph.depth_interpolated.values, winter_ph.salinity.values, **kwargs)  
    ...
    pf.xsection(fig, ax13, latesummer.time.values, latesummer.depth_interpolated.values, placeholder, **kwargs)  # latesummer DO

    kwargs['ylabel'] = None
    pf.xsection(fig, ax14, fall_do.time.values, fall_do.depth_interpolated.values, fall_do.oxygen_concentration_shifted_mgL.values, **kwargs)  # fall DO
