...
def main(savedir):
    ...
    pf.xsection(fig, ax9, latesummer.time.values, latesummer.depth_interpolated.values, latesummer.oxygen_concentration_shifted_mgL.values, **kwargs)  # latesummer DO

    kwargs['ylabel'] = None
    falldo_mgL = fall_do.oxygen_concentration_shifted.values * 32 / 1000  # convert from umol/L to mg/L
    pf.xsection(fig, ax10, fall_do.time.values, fall_do.depth_interpolated.values, falldo_mgL, **kwargs)  # fall DO
